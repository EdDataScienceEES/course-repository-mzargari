...
elongation[6, ]$Indiv   
# returns the value in the column Indiv for the sixth observation
...
# changes the names of the columns (getting rid of capital letters) 
# and overwriting our data frame
...
# Note: the select() function often clashes with functions of the same name in 
# other packages, and for that reason it is recommended to always use the 
...
# Corresponds to the sum of all growth increments in 
# the dataset (all individuals and years)

summary2 <- summarise(elong_grouped, total.growth = sum(length))
# gives us a breakdown of total growth per individual, our grouping variable
...
experiment2 <- merge(elongation_long, treatments, by.x = c("zone", "indiv"), 
                     by.y = c("Zone", "Indiv"))  
# this gives the same result as the left join
